...
The README used to carry Mermaid, which GitHub controls too much of: it decodes
HTML entities before parsing, ignores `%%{init}%%`, pins its own version and
picks the theme. What replaces it is `tools/gen_diagram.py`, which draws each
diagram twice, one file per colour scheme, served from a `<picture>` element.

The tests below are the obligations that come with that: the pictures are what
the generator draws, they reference files that exist, they carry nothing
GitHub strips, and Mermaid does not quietly come back.
"""
import importlib
import pathlib
import re
...
def test_every_diagram_offers_both_schemes_and_alt_text():
    ...
    for page in PAGES:
        ...
        for block in re.findall(r"<picture>.*?</picture>", text, re.DOTALL):
...
def test_mermaid_has_not_come_back():
    """The drawn diagrams replaced Mermaid; a new block would bring back the
    rendering GitHub controls."""
    found = [str(p.relative_to(ROOT)) for p in PAGES if MERMAID.search(p.read_text(encoding="utf-8"))]
...
def test_there_are_diagrams_to_check():
    ...
    module = _generator()
    assert len(module.DIAGRAMS) >= 2
    pictures = sum(len(re.findall(r"<picture>", p.read_text(encoding="utf-8"))) for p in PAGES)
    assert pictures >= 2, f"only {pictures} <picture> blocks found across the docs"
